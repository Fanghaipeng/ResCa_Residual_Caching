...
LOCAL_CHECKPOINT_ROOT = os.getenv("RESCA_CHECKPOINT_ROOT", os.path.join(PROJECT_ROOT, "checkpoints"))
LOCAL_FLUX_DEV_DIR = os.getenv(
    "FLUX_DEV_DIR",
    os.path.join(LOCAL_CHECKPOINT_ROOT, "black-forest-labs/FLUX_1-dev"),
)


def local_file_or_none(path: str) -> str | None:
    return path if os.path.exists(path) else None


LOCAL_T5_PATH = (
    os.getenv("T5_PATH")
    or local_file_or_none(os.path.join(LOCAL_CHECKPOINT_ROOT, "google/t5-v1_1-xxl"))
    or "google/t5-v1_1-xxl"
)
LOCAL_CLIP_PATH = (
    os.getenv("CLIP_PATH")
    or local_file_or_none(os.path.join(LOCAL_CHECKPOINT_ROOT, "openai/clip-vit-large-patch14"))
...
configs = {
    "flux-dev": ModelSpec(
        ...
        ckpt_path=os.getenv("FLUX_DEV") or local_file_or_none(os.path.join(LOCAL_FLUX_DEV_DIR, "flux1-dev.safetensors")),
        lora_path=None,
        params=FluxParams(
            in_channels=64,
            out_channels=64,
        ...
        ae_path=os.getenv("AE") or local_file_or_none(os.path.join(LOCAL_FLUX_DEV_DIR, "ae.safetensors")),
        ae_params=AutoEncoderParams(
            resolution=256,
            in_channels=3,
            ch=128,
