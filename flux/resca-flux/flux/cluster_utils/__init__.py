...
def get_cluster_info(X, cache_dic, current):
    ...
    cluster_info = cache_dic['cluster_info'][current['stream']][current['module']].copy()
    cluster_num = min(cluster_num, X.shape[1])

    if cache_dic['mode'] == 'ResCa':
    ...
    cache_dic['cluster_info'][current['stream']][current['module']] = cluster_info
