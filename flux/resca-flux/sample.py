...
def main(opts: SamplingOptions, model_kwargs: dict):
    ...
    if opts.use_nsfw_filter:
        nsfw_classifier = pipeline(
            ...
            model=os.getenv("NSFW_MODEL", "Falconsai/nsfw_image_detection"),
            device=device
        )
    else:
...
def app():
    ...
    parser.add_argument('--output_dir', type=str, default='outputs/flux_samples', help='Directory to save images.')
    parser.add_argument('--max_prompts', type=int, default=None, help='Only use the first N prompts from the prompt file.')
    ...
    parser.add_argument('--mode', type=str, default='ResCa', choices=['Taylor-Cache', 'ToCa', 'Taylor', 'ClusCa', 'ResCa'], help='Cache mode.')
    ...
    parser.add_argument('--tet_alpha', type=float, default=0.6, help='Temporal smoothing weight for ResCa TETC.')
    parser.add_argument('--resca_solver', type=str, default='ie', choices=['ie', 'it', 'bdf2'], help='ResCa sparse-step update solver.')
    parser.add_argument('--resca_proxy_method', type=str, default='center', choices=['center', 'random', 'center-random'], help='Proxy token selection method for ResCa.')
    ...
    model_kwargs = {
        ...
        'resca_solver': args.resca_solver,
        'resca_proxy_method': args.resca_proxy_method,
    }
