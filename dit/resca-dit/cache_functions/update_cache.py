...
def _resca_context(fresh_indices, fresh_tokens, cache_dic, current, max_order=1):
    ...
    k = cluster_info['k']
    if k != 1:
        raise ValueError("ResCa expects exactly one proxy token per cluster (k=1).")

    old_cache_dict = cache_dic['cache'][-1][layer][module]
...
def _resca_implicit_taylor_tokens(fresh_indices, fresh_tokens, cache_dic, current, max_order=1, eps=1e-6):
    ...
    for order in range(1, max_order + 1):
        proxy_order_next = proxy_orders_t[order - 1] - proxy_order_next
        ...
        x_next.add_(estimated_order, alpha=((-1.0) ** order) / math.factorial(order))
...
def _resca_first_order_residual(fresh_indices, fresh_tokens, cache_dic, current, eps=1e-6):
    ...
    proxy_residual_next = proxy_orders_t[0] - fresh_tokens
    estimated_residual = _resca_estimate_order(
        residual_t=cached_orders[1],
        proxy_residual_t=proxy_orders_t[1],
        proxy_residual_next=proxy_residual_next,
        cluster_index=cluster_index,
        x_dtype=x_t.dtype,
        eps=eps,
    )
    return x_t, cached_orders[1], estimated_residual, proxy_index, fresh_tokens
...
def resca_simulate_tokens(fresh_indices, fresh_tokens, cache_dic, current, eps=1e-6):
    ...
    `ie` is the first-order implicit Euler reduction, `it` uses residual
    orders 1..max_order, and `bdf2` applies the first-order BDF2-style update.
    """
    ...
    if solver == 'it':
        ...
        x_future = x_t + residual_t
        x_next = (4.0 / 3.0) * x_t - (1.0 / 3.0) * x_future - (2.0 / 3.0) * estimated_residual
