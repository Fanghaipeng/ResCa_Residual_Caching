...
from cache_functions import Attention, cal_type, cache_cutfresh, update_cache, propagation_update_cache, resca_simulate_tokens, force_init
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c, current, cache_dic):
        ...
        if current['type'] == 'full':  # Force Activation: Compute all tokens and save them in cache
            ...
            fresh_indices, fresh_tokens = cache_cutfresh(cache_dic, x, current)
            fresh_tokens = self.mlp(modulate(self.norm2(fresh_tokens), shift_mlp, scale_mlp))
            mlp_output = resca_simulate_tokens(
            ...
            # MLP FLOPs for proxy tokens.
            if test_FLOPs:
                B_fresh, N_fresh, C_fresh = fresh_tokens.shape
                mlp_hidden_dim = int(C_fresh * 4)
                flops += B_fresh * N_fresh * C_fresh * mlp_hidden_dim * 2
                flops += B_fresh * N_fresh * mlp_hidden_dim * C_fresh * 2
                flops += B_fresh * N_fresh * mlp_hidden_dim * 6
